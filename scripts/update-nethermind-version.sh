...
# Nethermind to the versions that release was built against. Nethermind's own
# assemblies are compiled against those (strong-named) versions, so a mismatch
# fails the build with CS1705 even when our own code needs no changes.
#
...
#
# Only strong-named packages can actually break the build: CS1705 is not raised

...
public static class DifficultyBombCalculator
{
    ...
    /// </summary>
    public const ulong InitialBombBlock = 200_000;

    /// <summary>
    /// The exponential difficulty period (100,000 blocks).
    /// </summary>
    public const ulong ExponentialPeriod = 100_000;
        ...
        ulong? ecip1041Block)
    {
        ...
        ulong period = blockNumber / ExponentialPeriod;

        // Gotham: bomb delayed
        if (gothamBlock is not null && blockNumber >= gothamBlock)
        {
            ulong bombDelay = (gothamBlock.Value - dieHardBlock.Value) / ExponentialPeriod;
            return period < bombDelay + 2
                ? BigInteger.Zero
        ...
        {
            ulong fixedPeriod = dieHardBlock.Value / ExponentialPeriod;
            return fixedPeriod < 2 ? BigInteger.Zero : BigInteger.Pow(2, (int)(fixedPeriod - 2));
        }

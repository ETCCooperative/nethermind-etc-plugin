...
public class EthashBaseTests
{
    ...
    [Test]
    public void IsValidPoWResult_Rejects_MatchingMixHash_WhenResultIsAboveTarget()
    {
        UInt256 difficulty = 2;

        EthashBase.IsValidPoWResult(
                mixHashMatches: true,
                ResultJustAboveDifficultyTwoTarget(),
                difficulty)
            .Should().BeFalse();
    }

    [Test]
    public void IsValidPoWResult_Accepts_MatchingMixHash_WhenResultMeetsTarget()
    {
        UInt256 difficulty = 2;

        EthashBase.IsValidPoWResult(
                mixHashMatches: true,
                ResultAtDifficultyTwoTarget(),
                difficulty)
            .Should().BeTrue();
    }

    [Test]
    public void IsValidMiningResult_Requires_MixHash_And_Target()
    {
        UInt256 difficulty = 2;

        EthashBase.IsValidMiningResult(
                mixHash: [0],
                hashimotoSuccess: true,
                ResultAtDifficultyTwoTarget(),
                difficulty)
            .Should().BeTrue();
    ...
    private static byte[] ResultAtDifficultyTwoTarget()
    {
    ...
    private static byte[] ResultJustAboveDifficultyTwoTarget()
    {
        byte[] result = ResultAtDifficultyTwoTarget();
        result[31] = 0x01;
        return result;
    }
}
